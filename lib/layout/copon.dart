...
import 'package:dirasti_dashboard/layout/choose_part.dart';
...
List<copon_module> selected_copon_list = [];
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        elevation: 0,
          backgroundColor: Colors.white,
          actions: [
        Text("قائمة أكواد التفعيل", style: TextStyle(fontSize: 30,color: Colors.black)),
        Spacer(),
            if(selected_copon_list.isNotEmpty)
            Row(
              children: [
                ElevatedButton(
                    onPressed: () {
                      showDialog(context: context, builder: (context)=>AlertDialog(
                        ...
                        ElevatedButton(onPressed: (){
                          selected_copon_list.forEach((element) {
                            dio.post_data(url: "/dash/delet_id",quary: {"table":" copon ","id":element.id}).then((value) {
                              print(value);
                              setState(() {
                                copon_list.remove(element);
                              });
                            });
                          });
                          selected_copon_list.clear();
                ...
                ElevatedButton(onPressed: () async {
                  for(var i =0 ;i<selected_copon_list.length; i++){
                    ui.Image image = await QrPainter(
                      data: selected_copon_list[i].uid_copon!,
                      gapless: true,
                    ...
                    download.download = selected_copon_list[i].uid_copon!+'.png';
                    download.click();
                    download.remove();
                  }
                }, child: Text("تنزيل qr")),
            ...
            ElevatedButton(
                ...
                onPressed: () {
                  Navigator.push(context, MaterialPageRoute(builder: (context)=>ChoosePart()));
                },
                child: Text("إضافة كود مخصص")),
            SizedBox(
              width: 30,
            ),
        ElevatedButton(
                  ...
                  builder: (context,setst) {
                    return AlertDialog(
                      ...
                      actions: [
                        ElevatedButton(onPressed: (){Navigator.pop(context);}, child: Text("إلفاء")),
                        ElevatedButton(onPressed: () async {
                          if(type==null || grade == null || tracher == null){
        ...
        ElevatedButton(
                      ...
                      actions: [
                        ...
                        ElevatedButton(onPressed: (){
                          ...
                          }else
                          {
                            ...
                            var l =
                                "$type|$grade|$subject|$tracher|${type == 0 ? file : course}";
                            for(var i=0;i<int.parse(uid_con.text);i++){
                              Random rng = Random();
                              var s = "";
                              for (var i = 0; i < 9; i++) {
                                if(rng.nextBool()){
            ...
            child: Text("إضافة عدد كبير من الأكواد")),
        SizedBox(
          width: 30,
        ),
        ElevatedButton(
                    ...
                    return AlertDialog(
                      ...
                      content: Column(
                        children: [
                          ...
                          DropdownButton(
                              value: type ,
                              items: ["ملفات","كورسات","اختبارات"].map((e) => DropdownMenuItem(child: Text(e),value: ["ملفات","كورسات","اختبارات"].indexOf(e),)).toList(),
                              //    value: grade_list[0].name,
                              onChanged: (value){
                                type = value;
                                grade = null;
                          ...
                          if(type == 2)
                            DropdownButton(
                                value: exam ,
                                items: exam_list.where((element) => element['grade']==grade && element['subject']==subject).map((e) => DropdownMenuItem(child: Text(e['name']!),value: e['id'].toString(),)).toList(),
                                //    value: grade_list[0].name,
                                onChanged: (value){
                                  exam = value;
                                  print(value);
                      ...
                      actions: [
                        ElevatedButton(onPressed: (){Navigator.pop(context);}, child: Text("إلفاء")),
                        ElevatedButton(onPressed: (){
                          if(type==null || grade == null || tracher == null || file == null&&course==null&&exam==null){
                            Tost_widget("يرجى ملئ المعلومات", "red");
                          }else {
                            var l =
                                "$type|$grade|$subject|$tracher|${type == 0 ? file : type==1?course:exam}";
                            dio.post_data(
                                url: "/dash/insert",
                                quary: {
                                  "table": " copon ",
...
Widget copon_list_element(BuildContext context, copon_module model, setstate) {
  ...
  try{
              ...
              child: Scrollbar(
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    Checkbox(value: selected_copon_list.contains(model), onChanged: (_){
                      if(_!){
                        selected_copon_list.add(model);
                      }else{
                        selected_copon_list.remove(model);
                      }
                      setstate((){});
                    }),
                    ...
                    SizedBox(
                      width: 30,
                    ), Center(child: Text(model.list_cours![0]=="0"?"ملف":model.list_cours![0]=="1"?"كورس":"اختبار")),
                    SizedBox(
                      width: 30,
                    ),
                    Center(child: Text("السنة:")),
                    ...
                    else if (model.list_cours![0].toString() == "1")
                      Center(child: Text("اسم الكورس:"))
                    else if (model.list_cours![0].toString() == "2")
                        Center(child: Text("اسم الاختبار:"))
                      else Center(child: Text("النوع: مادة كاملة")),
                    SizedBox(
                      width: 10,
                    ),
                    if(model.list_cours![0].toString() == "0")
                     Center(child: Text(file_list.where((element) => element.id == model.list_cours![4]).first.name??"غير متوفر")),
                      if(model.list_cours![0].toString() == "1")
                       Center(child: Text(course_list.where((element) => element.id == model.list_cours![4]).first.name??"غير متوفر")),
                        if(model.list_cours![0].toString() == "2")
                          Center(child: Text(exam_list.where((element) => element['id'].toString() == model.list_cours![4]).first['name']??"غير متوفر")),
                                  ...
                                  return AlertDialog(
                                    ...
                                    content: Column(
                                      children: [
                                        ...
                                        DropdownButton(
                                            value: type ,
                                            items: ["ملفات","كورسات","اختبارات"].map((e) => DropdownMenuItem(child: Text(e),value: ["ملفات","كورسات","اختبارات"].indexOf(e).toString(),)).toList(),
                                            //    value: grade_list[0].name,
                                            onChanged: (value){
                                              type = value.toString();
                                              grade = null;
                                    ...
                                    actions: [
                                      ...
                                      ElevatedButton(onPressed: (){
                                        /// TODO
                                        var l = "$type|$grade|$subject|$tracher|${type=='0'?file:type=="1"?course:exam}";
    ...
    return Container(color: Colors.red.withOpacity(0.3),height: 40,
    child: Row(
      children: [
        SizedBox(width: 50,),
        Text("خطأ في العنصر"),
